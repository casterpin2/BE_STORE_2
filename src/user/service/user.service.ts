...
export class UserService {
  ...
  public async login(user: UserModel): Promise<ResponseViewModel> {
    try {
      ...
      const sql = 'select id,password from masteruser where username=$1 ';
      ...
      const match = bcrypt.compareSync(user.password + `s04$$w0rD`, userDb.password as string);
  ...
  public async createUser(user: UserModel): Promise<ResponseViewModel> {
    ...
    try {
      ...
      const sql =
        'INSERT INTO masteruser(username, password, firstname,lastname) VALUES($1, $2, $3::text,$4::text)';
